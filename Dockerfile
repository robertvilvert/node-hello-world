...
RUN rm /bin/sh && ln -s /bin/bash /bin/sh && apt-get update && apt-get install wget -y && apt-get install unzip -y && apt-get clean && apt-get install -y tar && apt-get install -y xz-utils && \
